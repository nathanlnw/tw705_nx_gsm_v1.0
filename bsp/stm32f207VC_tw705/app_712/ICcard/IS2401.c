...
{  
  ...
  {  
	 ...
	 {  
	  ...
	  if(!IIC_WriteByte(0xa0)){IIC_Stop();return FAILED;} 
	  if(!IIC_WriteByte(addr)){IIC_Stop();return FAILED;} 
	  if(!IIC_WriteByte(byte)){IIC_Stop();return FAILED;} 
	  IIC_Stop(); 
	  return PASS; 
	  ...
	  if(!IIC_WriteByte(0xa0)){IIC_Stop();return FAILED;} 
	  if(!IIC_WriteByte(addr)){IIC_Stop();return FAILED;} 
	  if(!IIC_Start()){IIC_Stop();return FAILED;}  
	  if(!IIC_WriteByte(0xa1)){IIC_Stop();return FAILED;} 
	  rt_kprintf(" + ");
